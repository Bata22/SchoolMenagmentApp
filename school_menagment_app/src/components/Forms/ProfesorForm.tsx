...
const schema = z.object({
  ...
  phone: z.number().min(1,{message: 'Phone is required and need to be an a number'}),
  address: z.string().min(1,{message: 'Address is required'}),
  bloodType: z.string().min(1,{message: 'Blood is required'}).max(3,{message:'Use one up to two letters and one sign + or -'}),
  birthday: z.date({message: 'Birthdayis required'}),
  sex: z.enum(["male", "female"]).optional(),
  img:z.instanceof(File,{message: 'Image is requried'})
  
});

type Inputs = z.infer<typeof schema>;

const ProfesorForm = ({type, data}:{type:"create" | "update",data?:any}) => {
...
const {
  ...
  const addProfesor = handleSubmit


    return (<form className='flex  flex-col gap-8' onSubmit={addProfesor(data=>{
        console.log(data)
    })} >
        <h1 className='text-xl font-semibold'>Add a new profesor </h1>
        ...
        <div className='flex flex-wrap gap-4 justify-between'>
          ...
          <InputField label='Birthday' name="birthday" type="date"  defaultValue={data?.birthday} register={register} error={errors.birthday} />
           ...
           <div className='flex flex-col gap-2 w-full md:w-1/4'>
            ...
            {errors.sex?.message && <p className='text-xm text-red-400'>{errors.sex.message.toString()}</p>}
        </div>
        <div className='flex flex-col gap-2 w-full md:w-1/4 items-center justify-center'>
            ...
            {errors.img?.message && <p className='text-xm text-red-400'>{errors.img.message.toString()}</p>}
        </div>
    </div>
